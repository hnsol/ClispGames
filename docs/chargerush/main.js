...
options = {
  ...
  // seed: 277,
  // seed: 292,
  // seed: 293, // good explosion
  seed: 301, // ambient taste
  isPlayingBgm: true,
...
function update() {
  ...
  stars.forEach((s) => {
    ...
    s.pos.y += s.speed;
    // Bring the star back to top when reach bottom
  ...
  remove(enemies, (e) => {
    ...
    color("black");
    // Check collision and draw the sprite
    ...
    // Check whether to make a small particle explosion
    if (isCollidingWithFBullets) {
      color("light_yellow");
      particle(e.pos, 8, 1.0);
      color("light_black");
      particle(e.pos, 8, 0.8);
      color("red");
      particle(e.pos, 8, 0.6);
      play("explosion");
