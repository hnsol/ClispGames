title = "Tears of the Kanagan [EASY]";
...
function update() {
  ...
  if (nextTrackTicks < 0) {
    ...
    tracks.push({
      ...
      vx: rnd(1, sqrt(difficulty)) * balance * (world > 0 ? -1 : 1),
      world,
    });
    trackCount++;
    nextTrackTicks = rnd(50, 60) / sqrt(difficulty) / balance;
  }
  remove(tracks, (t) => {
    t.x += t.vx;
  ...
  if (nextDiaTicks < 0) {
    ...
    dias.push({
      ...
      vx: (rnd(1, sqrt(difficulty)) * balance * (diaWorld > 0 ? -1 : 1)) / 3,
      world: diaWorld,
    });
    nextDiaTicks = rnd(120, 150) / sqrt(difficulty) / balance;
  }
  remove(dias, (d) => {
    d.pos.x += d.vx;
    // color("light_yellow");
    ...
    if (d.world > 0 ? d.pos.x < 103 : d.pos.x > 97) {
      play("explosion");
      color("red");
      text("X", 100, d.pos.y, {scale: {x: 3, y: 3}});
      end();
    }
  });
}
